...
const SkeletonRow = ({ i }) => (
    <tr className="border-b border-slate-50">
        <td className="py-3.5 px-4">
            <div className="h-3 bg-slate-100 rounded animate-pulse" style={{ width: `${140 + (i % 6) * 30}px`, animationDelay: `${i * 35}ms` }} />
        </td>
        {[0, 1, 2, 3].map(j => (
...
export default function QueriesPage() {
    ...
    useEffect(() => {
        ...
        api.get(`/auth/gsc/queries/${encodeURIComponent(selectedProperty)}`, { params: { days } })
            .then(res => {
                const d = res.data.queries || [];
                ssSet(cacheKey, d);
                setRawPages(d);
            })
            .catch(err => toast.error(err.response?.data?.detail || 'Failed to load queries'))
        ...
        <div className="min-h-screen bg-[#f5f6f8]">
            ...
            <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-slate-100 sticky top-0 z-20">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-[17px] font-black text-slate-900">Queries</h1>
                    {!loading && (
                        ...
                        <table className="w-full text-left whitespace-nowrap">
                            <thead className="bg-slate-50 border-b border-slate-100 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                                <tr>
                                    <th className="py-3 px-4 text-slate-600">
                                        <button onClick={() => handleSort('query')} disabled={loading} className="hover:text-slate-800 transition-colors">
                                            Query <SortIcon col="query" sortKey={sortKey} sortDir={sortDir} />
                                        </button>
                                    </th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {loading ? (
                                    ...
                                    >
                                        <td className="py-3.5 px-4">
                                            <span className="text-[13px] font-semibold text-slate-700 truncate block max-w-[480px]" title={row.query}>
                                                {row.query}
                                            </span>
                                        </td>
