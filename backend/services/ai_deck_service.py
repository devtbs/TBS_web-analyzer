...
GSC_STRUCTURE = (
    ...
    "'Unique keywords (distinct queries)' number, plus a SMALL short-tail vs long-tail DONUT (Plotly pie, \"hole\":0.6, "
    "values = [short-tail count, long-tail count] from KEYWORD MIX, labels = [\"Short-tail (1-2 words)\",\"Long-tail (3+ words)\"], "
    "colours = [accent, accent-2], sized ~180x180 to fit its box with a slim legend below — must NOT overflow/clip). "
    "RIGHT column (~62%, the HERO): a Plotly BUBBLE/scatter from KEYWORD POSITION vs IMPRESSIONS: x = average position (REVERSED axis), "
    "y = impressions, marker size ∝ impressions. "
    "REQUIRED: directly label the THREE highest-impression bubbles — add a \"layout\":{\"annotations\":[...]} entry for each of those 3 "
    "queries with x = its avg position, y = its impressions, \"text\" = the query, \"showarrow\":true and a short thin leader line in the "
